...
import com.yx.DBSCANClustering.imis.Imis_DBSCAN
...
object ImisTrajectoryETL_NOG {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //Spark应用的基础配置
    val conf = new SparkConf().setAppName("ImisTrajectoryETL_NOG")
    ...
    val DbscanRDD = parsedLogRDD
      ...
      .flatMapValues { DbscanArrayBuffer =>
        val gDBSCAN = new Imis_DBSCAN(eps, 10, DbscanArrayBuffer.toArray)
        gDBSCAN.cluster()
      }

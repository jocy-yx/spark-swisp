...
object AisTrajectoryETL_Stage {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //Spark应用的基础配置
    val conf = new SparkConf().setAppName("AisTrajectoryETL_Stage")
    ...
    //数据集
//    val testTrajectoryRawInputPath = conf.get("spark.AisTrajectoryETL_Stage.testTrajectoryRawInputPath",
//      "E:\\data\\Ais\\ais_04.txt")
    val testTrajectoryRawInputPath = args(0)

    //输出
//    val OutputPath = conf.get("spark.AisTrajectoryETL_Stage.OutputPath", "data/ais/")
    val OutputPath = args(1)

    val outputFileType = "text"
    val dataTime = conf.get("spark.AisTrajectoryETL_Stage.dataTime", "20170104")
...
//    val m = 15
    ...
    val parsedLogRDD: RDD[TrajectoryData] = sc
      ...
      .flatMap(line => RawDataAisParser.parse(line,eps))
    ...
    val mapperTrajectory: RDD[(String, Array[String])] = phoneSingleRDD
      ...
      .mapValues { arrayTrajectory => //4.2 对映射轨迹进行填充

package com.yx.trajectory.imis

import com.yx.DBSCANClustering.imis.{Imis_DBSCAN, Imis_GDBSCAN}
...
object ImisTrajectoryETL_G {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //Spark应用的基础配置
    val conf = new SparkConf().setAppName("ImisTrajectoryETL_G")
    ...
    // "E:\\data\\ISIM\\imis1month5.txt")
    val testTrajectoryRawInputPath = args(0)

    //输出
    //val OutputPath = conf.get("spark.TrajectoryETL.OutputPath", "data/imis/")
    ...
    val parsedLogRDD: RDD[TrajectoryData] = sc
      ...
      .flatMap(line => RawDataImisParser.parse(line, eps)) 
    ...
    val DbscanRDD = parsedLogRDD
      ...
      .flatMapValues { DbscanArrayBuffer =>
        val gDBSCAN = new Imis_GDBSCAN(eps, 10, DbscanArrayBuffer.toArray)
        gDBSCAN.cluster()
      }
    ...
    /* //3.形成Key-Value键值对数据格式，并且Key为用户的的手机号
     ...
     // (abc,(09:15,1,0.8717263713318235,0.5665615867089149))

...
import com.yx.DBSCANClustering.ais.Ais_GDBSCAN
...
object AisTrajectoryETL {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //Spark应用的基础配置
    val conf = new SparkConf().setAppName("AisTrajectoryETL")
    ...
    //数据集
//    val testTrajectoryRawInputPath = conf.get("spark.AisTrajectoryETL.testTrajectoryRawInputPath",
//      "E:\\data\\Ais\\ais_04.txt")
    val testTrajectoryRawInputPath = args(0)

    //输出
//    val OutputPath = conf.get("spark.AisTrajectoryETL.OutputPath", "data/ais/")
    val OutputPath = args(1)

    val outputFileType = "text"
    val dataTime = conf.get("spark.AisTrajectoryETL.dataTime", "20170104")
...
//    val m = 15
    ...
    val parsedLogRDD: RDD[TrajectoryData] = sc
      ...
      .flatMap(line => RawDataAisParser.parse(line,eps))//更改******************
    ...
    val DbscanRDD = DbscanRDD0
      .flatMapValues{DbscanArrayBuffer =>
        val gDBSCAN = new Ais_GDBSCAN(eps,10,DbscanArrayBuffer.toArray)
        gDBSCAN.cluster()
      }
    ...
    val phoneSingleRDD:RDD[(String,RawTrajectory)] = phoneGroupedRDD.mapValues { value =>
      val processor = new OneTrajectoriesProcessor(value._1,value._2)//更改************

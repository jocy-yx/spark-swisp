...
import com.yx.DBSCANClustering.ais.Ais_DBSCAN
...
object AisTrajectoryETL_NOG {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //Spark应用的基础配置
    val conf = new SparkConf().setAppName("AisTrajectoryETL_NOG")
    ...
    //数据集
//    val testTrajectoryRawInputPath = conf.get("spark.AisTrajectoryETL_NOG.testTrajectoryRawInputPath",
...
//    val OutputPath = conf.get("spark.TrajectoryETL.OutputPath", "data/geolife/")
    ...
    val dataTime = conf.get("spark.AisTrajectoryETL_NOG.dataTime", "20170104")
...
//        val m = 15
    ...
    val DbscanRDD = DbscanRDD0
      .flatMapValues { DbscanArrayBuffer =>
        val gDBSCAN = new Ais_DBSCAN(eps, 10, DbscanArrayBuffer.toArray)
        gDBSCAN.cluster()
      }

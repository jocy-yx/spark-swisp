...
//import com.yx.DBSCAN.Java.{Dbscan, Point, Utility}
//import com.yx.spark.trajectory.Utils.StringTest
...
//
//    //Spark应用的基础配置
//    val conf = new SparkConf().setAppName("TrajectoryETL")
...
//      "E:\\大数据时空数据分布式伴随研究\\数据集\\trucks\\trucks-新\\Trucks1\\trucks0910.csv")
//
//    //输出
//    val OutputPath = conf.get("spark.TrajectoryETL.OutputPath", "data/trucks/")
//
//    val outputFileType = if (args.nonEmpty) args(0) else "text"
//    val dataTime = conf.get("spark.TrajectoryETL.dataTime", "20021009")
...
//      .flatMap(line => RawDataParser.parse(line))//更改******************
//
//    //2.增加优化后的DBSCAN算法
//    val DbscanRDD = parsedLogRDD
//      .map(trajectoryData =>
//        (trajectoryData.getDateTime.toString.substring(10,15), trajectoryData))//更改******************
//      .combineByKey(createCombinerDBSCAN,mergeValueDBSCAN,mergeCombinersDBSCAN)
//      .flatMapValues{DbscanArrayBuffer=>
//        Dbscan.applyDbscan(DbscanArrayBuffer.toArray)
...
//      .map(point => (point._2.getUser,
//        (point._1,point._2.getClusterID,point._2.getX,point._2.getY)))
//    // 字段为：key（用户） value：（时间戳、clusterID、经度、纬度）
//    // (abc,(10/09/200209:15:59,1,0.8717263713318235,0.5665615867089149))
//
//    //4.对每一个用户（即每一个phone）的所有信息进行轨迹与基站映射
//    val phoneSingleRDD:RDD[(String,RawTrajectory)] = phoneGroupedRDD.mapValues { value =>
//      val processor = new OnePhoneTrajectoriesProcessor(value._1,value._2,value._3,value._4)//更改************
...
//        val processor = new bucketProcessor(iterableMapperTrajectory)
//        processor.buildBucket()//可优化#####
//      }
//      .filter { case (_, value: Bucket) => ResultDataPro.pro(value.getPath.toString) }//可优化#####
//
//
...
//    } //此处用这种写法，请注意，后期要考虑误差率40%的问题
//      .combineByKey(createCombinerGroup, mergeValueGroup, mergeCombinersGroup,1) //=====users合并
...
//      }
//      .combineByKey(createCombinerGroupSim, mergeValueGroupSim, mergeCombinersGroupSim,1)//===出错
...
//      .mapValues(arrayBuffer => arrayBuffer.distinct)
//      .filter { case (key: (Int, String), value: Array[String]) => value.length >= 2 }
...
//      .combineByKey(createCombinerGroup, mergeValueGroup, mergeCombinersGroup,1)//===========
//      .mapValues { value =>
...
//    println("程序运行时间："+(endTime-startTime)/1000+"s")
//
//

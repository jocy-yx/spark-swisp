...
import com.yx.DBSCANClustering.trucks.{Trucks_DBSCAN, Trucks_GDBSCAN}
...
object TrucksTrajectoryETL_NOG {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //Spark应用的基础配置
    val conf = new SparkConf().setAppName("TrucksTrajectoryETL_NOG")
...
//    val OutputPath = conf.get("spark.TrajectoryETL.OutputPath", "data/trucks/")
    ...
    val parsedLogRDD: RDD[TrajectoryData] = sc
      ...
      .flatMap(line => RawDataTruckParser.parse(line,eps))
    ...
    val DbscanRDD = parsedLogRDD
      ...
      .flatMapValues{DbscanArrayBuffer=>
        val gDBSCAN = new Trucks_DBSCAN(eps,10,DbscanArrayBuffer.toArray)
        gDBSCAN.cluster()
      }

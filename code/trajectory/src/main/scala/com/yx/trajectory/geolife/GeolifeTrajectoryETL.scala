...
object GeolifeTrajectoryETL {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //Spark应用的基础配置
    val conf = new SparkConf().setAppName("GeolifeTrajectoryETL")
    ...
    //数据集
//    val testTrajectoryRawInputPath = conf.get("spark.GeolifeTrajectoryETL.testTrajectoryRawInputPath",
...
//    //E:\data\GeoLife\dayData20090220\geolife20090220.csv
    ...
    //输出
//    val OutputPath = conf.get("spark.GeolifeTrajectoryETL.OutputPath", "data/geolife/")
    val OutputPath = args(1)

    val outputFileType = "text"
    val dataTime = conf.get("spark.GeolifeTrajectoryETL.dataTime", "20021009")
...
//    val m = 15
    ...
    val parsedLogRDD: RDD[TrajectoryData] = sc
      ...
      .flatMap(line => RawDataGeolifeParser.parse(line,eps))//更改******************
    ...
    val DbscanRDD = DbscanRDD0
      .flatMapValues{DbscanArrayBuffer =>
        val gDBSCAN = new GeoLife_GDBSCAN(eps,10,DbscanArrayBuffer.toArray)
        gDBSCAN.cluster()
      }
    ...
    val expandedRDD: RDD[((Int, String), Array[String])] = bucketPhoneRDD
      ...
      .combineByKey(createCombinerGroup, mergeValueGroup, mergeCombinersGroup) //=====users合并
      ...
      .combineByKey(createCombinerGroupSim, mergeValueGroupSim, mergeCombinersGroupSim)//===出错
    ...
    val comBucketSecondRDD: RDD[(String, String, String, String)] = comBucketOneRDD
      ...
      .mapValues { value =>
        ...
        val pathSplit: String = strSplit(strSplit.length - 1)
    ...
    val coalesceRDD = comBucketSecondRDD.coalesce(1, false) 

package com.yx.DBSCANClustering.geolife;
...
public class GeoLife_GDBSCAN {
    ...
    // 构造方法2
    public GeoLife_GDBSCAN(double eps, int minPts, Point[] points) {
        this.eps = eps;
        this.minPts = minPts;
        for (Point p :points){
...
//        clusters.add(noisies);
    ...
    private ArrayList<String> getNeighbors(String point) {
        ...
        GeoLifeGrid geoLifeGrid = new GeoLifeGrid(eps);
        List<Long> closeIds = geoLifeGrid.closeId(rawData.get(point).getCellID());
